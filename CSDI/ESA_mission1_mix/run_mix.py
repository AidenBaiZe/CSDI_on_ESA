...
from model import RandomStructuredMixCSDI  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="ESA Mission 1 random-structured mixed masking experiment"
    )
    parser.add_argument(
        "--mode",
        choices=(
...
def create_protocols(config: dict[str, Any], config_path: Path) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "training_random_point_masks": float(
            config["mix_mask"]["random_probability"]
        )
        > 0.0,
...
def run_preflight(config: dict[str, Any], config_path: Path) -> dict[str, Any]:
    ...
    if config["model"]["target_strategy"] != "random_structured_mix":
        raise ValueError(
            "model.target_strategy must be 'random_structured_mix' for this experiment"
        )
    random_reference = resolve_path(
        config["references"]["random_evaluation_dir"], config_path
    )
    structured_reference = resolve_path(
        config["references"]["structured_evaluation_dir"], config_path
    )
    for path in (
        random_reference / "metrics_missing_10.json",
        random_reference / "metrics_missing_50.json",
        random_reference / "metrics_missing_90.json",
        structured_reference / "metrics" / "structured_only_csdi" / "summary.json",
    ):
        if not path.is_file():
            raise FileNotFoundError(f"Reference result is missing: {path}")
    starts, _, _ = _load_reference_windows(config, config_path)
    configured_families = set(config["mix_mask"]["structured_family_probabilities"])
    if configured_families != set(FAMILY_NAMES):
        raise ValueError(
            "Mix training config must contain exactly the three structured families "
    ...
    if not 0.0 <= random_probability <= 1.0:
        raise ValueError("mix_mask.random_probability must be in [0, 1]")
    payload = {
        ...
        "mix_training_steps": int(config["train"]["steps"]),
        "training_families": ["random", *FAMILY_NAMES],
        "random_probability": random_probability,
        "structured_family_probabilities": config["mix_mask"][
            "structured_family_probabilities"
        ],
        "random_point_masks_used_for_training": random_probability > 0.0,
...
def run_smoke_cpu(config: dict[str, Any], config_path: Path) -> dict[str, Any]:
    ...
    model = make_model(config, torch.device("cpu"), "mix_csdi")
    ...
    if not torch.isfinite(loss):
        raise FloatingPointError("CPU random-structured mix smoke loss is non-finite")
    payload = {
...
def run_smoke_train(config: dict[str, Any], config_path: Path) -> dict[str, Any]:
    ...
    set_global_seed(int(config["train"]["seed"]))
    model = make_model(config, device, "mix_csdi")
...
def run_smoke_suite(config: dict[str, Any], config_path: Path) -> dict[str, Any]:
    ...
    configured_chunk = int(config["evaluation"]["sample_chunk_size"])
    selected = next(
        (
            row
            for row in successful
            if int(row["chunk_size"]) == configured_chunk
        ),
        None,
    )
    if selected is None:
        raise RuntimeError(
            f"Configured sample chunk {configured_chunk} did not pass the smoke test"
        )
    failed_chunks = []
    ...
    write_json(
        ...
        {
            "sample_chunk_size": int(selected["chunk_size"]),
            "source": "configured_and_smoke_verified",
        },
    )
    return payload
...
def run_training(
    ...
    train_store, _ = load_stores(config, config_path)
    output = resolve_path(config["models"]["mix_output_dir"], config_path)
    final_step = int(config["train"]["steps"])
    final_checkpoint = output / f"checkpoint_step_{final_step:06d}.pth"
    set_global_seed(int(config["train"]["seed"]))
    model = make_model(
        config, torch.device(config["train"]["device"]), "mix_csdi"
    )
    if final_checkpoint.is_file():
        ...
        write_json(output / "mix_mask_statistics.json", model.mask_statistics())
        return summary
...
def run_report(config: dict[str, Any], config_path: Path) -> None:
    from report_mix import generate_report
...
def main() -> None:
    ...
    try:
        if args.mode == "preflight":
        ...
        elif args.mode == "evaluate-mix":
            print(json.dumps(evaluate_protocols(config, config_path, "mix_csdi", args.force_evaluation), indent=2, ensure_ascii=False))
            ...
            write_status(config, config_path, "running", "smoke", "Running CPU/GPU mixed-mask smoke checks")
            run_smoke_suite(config, config_path)
            write_status(config, config_path, "running", "train_mix", "Training Random-Structured-Mix-CSDI to 32,000 steps")
            run_training(config, config_path, args.resume)
            write_status(config, config_path, "running", "evaluate_mix", "Evaluating Random-Structured-Mix-CSDI on 11 structured and 3 random protocols")
            evaluate_protocols(config, config_path, "mix_csdi", args.force_evaluation)
            write_status(config, config_path, "running", "report", "Generating the comparison report and figures")
            run_report(config, config_path)
            write_status(config, config_path, "complete", "complete", "All mixed masking training, evaluations, and reporting are complete")
