...
$result = Join-Path $here "results\mission1_random_structured_mix_seed1"
...
while (Get-Process -Name "TheBazaar" -ErrorAction SilentlyContinue) {
    Write-WaitStatus "TheBazaar is using the GPU; the Mix experiment will start automatically after it exits"
...
python "$here\run_mix.py" --mode full --resume 2>&1 |
