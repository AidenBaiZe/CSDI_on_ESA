...
CHANNEL_RE = re.compile(r"^ESA-Mission1/channels/channel_(\d+)\.zip$")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preprocess all ESA-ADB Mission 1 telemetry channels for CSDI."
    )
...
def discover_channel_entries(
    archive: Path, expected_count: int
    ...
    if len(channel_ids) != expected_count:
        raise ValueError(
            f"Expected {expected_count} Mission 1 telemetry channels, found "
            f"{len(channel_ids)}: {channel_ids}"
        )
    expected_ids = list(range(1, expected_count + 1))
    if channel_ids != expected_ids:
        raise ValueError(
            "Mission 1 channel identifiers are not the expected archive-defined "
...
def load_annotation_tables(
    archive: Path, labels_fallback: Path, anomaly_types_fallback: Path
) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, str]]:
    labels, labels_source = _read_csv_from_archive_or_fallback(
        archive, "ESA-Mission1/labels.csv", labels_fallback
    )
    anomaly_types, types_source = _read_csv_from_archive_or_fallback(
        archive, "ESA-Mission1/anomaly_types.csv", anomaly_types_fallback
    )
...
def read_channel(path: Path, channel_name: str) -> pd.DataFrame:
    ...
    frame = pd.DataFrame({"value": values.to_numpy(dtype=np.float64)}, index=index)
...
def apply_derivative(frame: pd.DataFrame, channel_id: int) -> pd.DataFrame:
    result = frame.copy()
    if 4 <= channel_id <= 11:
...
def classify_channel(
    ...
    differenced = 4 <= int(channel_id) <= 11
...
def preprocess(config: dict[str, Any], config_path: Path, force: bool, skip_hash: bool) -> Path:
    ...
    if not archive.is_file():
        raise FileNotFoundError(f"Mission 1 archive not found: {archive}")
    ...
    channel_ids, entries = discover_channel_entries(archive, expected_count)
    ...
    expected_timepoints = int(data["expected_timepoints_per_split"])
    for name, index in (("train", train_index), ("test", test_index)):
        if len(index) != expected_timepoints:
            raise ValueError(
                f"Official {name} index has {len(index)} points, expected {expected_timepoints}"
            )
    ...
    for column, channel_id in enumerate(tqdm(channel_ids, desc="preprocess channels")):
        ...
        raw = read_channel(channel_paths[channel_id], channel_name)
        raw = apply_derivative(raw, channel_id)
        ...
        unique_count, discrete_like, differenced = classify_channel(
            clean_values, channel_id, int(data["discrete_unique_threshold"])
        )
        means[column] = mean
    ...
    manifest = {
        "schema_version": 2,
        "source": {
            "dataset": "ESA Anomaly Dataset / ESA-ADB Mission 1",
            "archive": str(archive),
        ...
        "esa_adb_compatibility": {
            ...
            "resample": "30-second zero-order hold, global ffill().bfill()",
            "anomaly_sample_restoration": True,
            "differenced_channel_ids": list(range(4, 12)),
        },
        "schema": {
        ...
        "partitions": {
            "train": {
                ...
                "raw_filter": "timestamp <= 2007-01-01T00:00:00",
            },
            "test": {
                "path": "test",
