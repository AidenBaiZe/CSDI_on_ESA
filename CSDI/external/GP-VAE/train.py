...
flags.DEFINE_integer('num_samples', 100, 'Number of posterior samples to save for probabilistic imputation evaluation')
...
def main(argv):
    ...
    if FLAGS.data_type == 'physionet' and "x_test_full" in data:
        x_val_full = x_test_full
        x_val_miss = x_test_miss
        m_val_miss = m_test_miss
        m_val_artificial = m_test_artificial
        y_val = y_test
    ...
    if FLAGS.data_type == "hmnist":
        ...
        if FLAGS.num_samples > 0:
            x_val_imputed_samples = []
            for i in range(FLAGS.num_samples):
                z_sample = [model.encode(x_batch).sample().numpy() for x_batch in x_val_miss_batches]
                x_val_imputed_sample = np.vstack([model.decode(z_batch).sample().numpy() for z_batch in z_sample])
                x_val_imputed_samples.append(x_val_imputed_sample)
            x_val_imputed_samples = np.stack(x_val_imputed_samples, axis=1)
            np.save(os.path.join(outdir, "imputed_samples_no_gt"), x_val_imputed_samples)
