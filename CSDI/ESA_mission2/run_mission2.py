...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ESA Mission 2 CSDI experiment")
    parser.add_argument(
        "--mode",
...
def make_model(
    config: dict[str, Any], device: torch.device, target_dim: int
) -> CSDI_Physio:
    model_config = {
        "model": copy.deepcopy(config["model"]),
        "diffusion": copy.deepcopy(config["diffusion"]),
    }
    return CSDI_Physio(model_config, device, target_dim=target_dim).to(device)
...
def run_preflight(config: dict[str, Any], config_path: Path) -> dict[str, Any]:
    ...
    expected_train = int(config["data"]["expected_train_timepoints"])
    expected_test = int(config["data"]["expected_test_timepoints"])
    if train_store.timestamps_ns.size != expected_train:
        raise ValueError("Processed train length does not match the official ESA-ADB grid")
    if test_store.timestamps_ns.size != expected_test:
        raise ValueError("Processed test length does not match the official ESA-ADB grid")
    if len(train_store.channel_names) != int(config["data"]["expected_channel_count"]):
        raise ValueError("Processed channel count does not match the configuration")
    ...
    payload = {
        ...
        "channel_count": len(train_store.channel_names),
        ...
        "training_passes_at_configured_steps": (
...
def run_smoke(
    ...
    length = int(config["data"]["window_length"])
    stride = int(config["data"]["stride"])
    target_dim = len(train_store.channel_names)
    ...
    cpu_model = make_model(config, torch.device("cpu"), target_dim)
    ...
    set_global_seed(int(config["train"]["seed"]))
    gpu_model = make_model(config, device, target_dim)
...
def benchmark_chunks(
    ...
    if model is None:
        device = torch.device(config["train"]["device"])
        model = make_model(config, device, len(test_store.channel_names))
        final_checkpoint = (
            resolve_path(config["train"]["output_dir"], config_path)
    ...
    smoke_nsample = int(config["smoke"]["nsample"])
    smoke_chunk = int(config["smoke"]["chunk_size"])
    # Warm-up pass so the first timed candidate does not absorb CUDA start-up costs.
    evaluate_model(
        model,
        loader,
        ratio=0.5,
        nsample=smoke_nsample,
        sample_chunk_size=smoke_chunk,
            ...
            results.append(
                {
                    "chunk_size": chunk,
                    "success": True,
                    "duration_seconds_one_full_sample_batch": result["duration_seconds"],
    ...
    # A larger chunk can silently spill into system memory and become an order of
    # magnitude slower, so select by measured duration rather than chunk size.
    chosen = min(safe, key=lambda value: value["duration_seconds_one_full_sample_batch"])
    requested_windows = int(evaluation["requested_windows"])
    batches = math.ceil(requested_windows / int(evaluation["batch_size"]))
    projected_seconds = (
        chosen["duration_seconds_one_full_sample_batch"]
        * batches
    ...
    payload = {
        ...
        "selection_rule": "fastest measured candidate within the memory ceiling",
        "results": results,
...
def run_training(config: dict[str, Any], config_path: Path, resume: bool) -> dict[str, Any]:
    ...
    set_global_seed(int(config["train"]["seed"]))
    model = make_model(
        config, torch.device(config["train"]["device"]), len(train_store.channel_names)
    )
