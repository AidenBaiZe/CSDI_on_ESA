...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device):
        ...
        self.random_mask_min_ratio = float(
            config["model"].get("random_mask_min_ratio", 0.0)
        )
        self.random_mask_max_ratio = float(
            config["model"].get("random_mask_max_ratio", 1.0)
        )
        self.mix_random_probability = float(
            config["model"].get("mix_random_probability", 0.5)
        )
        self.use_provided_hist_mask = bool(
            config["model"].get("use_provided_hist_mask", False)
        )
        if not 0.0 <= self.random_mask_min_ratio <= self.random_mask_max_ratio <= 1.0:
            raise ValueError("random mask ratios must satisfy 0 <= min <= max <= 1")
        if not 0.0 <= self.mix_random_probability <= 1.0:
            raise ValueError("mix_random_probability must be in [0, 1]")
    ...
    def get_randmask(self, observed_mask):
        ...
        for i in range(len(observed_mask)):
            sample_ratio = np.random.uniform(
                self.random_mask_min_ratio, self.random_mask_max_ratio
            )
    ...
    def get_hist_mask(self, observed_mask, for_pattern_mask=None):
        ...
        for i in range(len(cond_mask)):
            ...
            if (
                self.target_strategy == "mix"
                and mask_choice < self.mix_random_probability
            ):
                cond_mask[i] = rand_mask[i]
            elif self.use_provided_hist_mask:
                cond_mask[i] = cond_mask[i] * for_pattern_mask[i]
...
class CSDI_Physio(CSDI_base):
    ...
    def process_data(self, batch):
        ...
        for_pattern_mask = batch.get("hist_mask", batch["observed_mask"])
